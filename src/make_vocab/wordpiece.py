...
class BytePairEncoder:
    ...
    def _build_subword_units(self, vocabs):
        ...
        for i in range(self.n_iters + 1):
            ...
            ### Modified ###
            vocab_new = set([v for vocab in vocabs.keys() for v in vocab.split()])
            unique_vocab_length = len(vocab_new)
            print("Current Vocab Length: {}".format(unique_vocab_length))
            print("Vocab to be created: {}".format(vocab_new - vocab_old))
            print("Vocab to be disappeared: {}".format(vocab_old - vocab_new))
            vocab_old = vocab_new
            if unique_vocab_length >= 30000: 
                break
